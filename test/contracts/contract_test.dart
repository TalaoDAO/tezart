...
void main() {
  ...
  final originateContractSetUp =
      (List<Map<String, dynamic>> code, dynamic storage) async {
        ...
        final originationOperation =
            operationsList.operations.last as OriginationOperation;
  ...
  group('#balance', () {
    ...
    group('when the contract address doesnt exist', () {
      ...
      test('it throws an error', () {
        // TODO(hawkbee): throw TezartNodeError or ContractError ?
        expect(
          () => subject(),
          throwsA(
            predicate((e) => e is TezartHttpError && e.message == 'Not Found'),
          ),
        );
      });
    });
  ...
  group('#storage', () {
    ...
    group('when the contract address doesnt exist', () {
      ...
      test('it throws an error', () {
        ...
        expect(
          () => subject(),
          throwsA(predicate((e) => e is TezartHttpError)),
        );
      });
    });
  });
      ...
      group('#callOperation', () {
        ...
        group('when the params are valid', () {
          ...
          test('it updates the storage value correctly', () async {
            ...
            final value = await bigMap.fetch(
              key: 'key',
              rpcInterface: rpcInterface,
            );
  ...
  group('#callOperation', () {
    ...
    group('when the params are invalid', () {
      group(
        'when the params are incompatible with the entrypoint signature',
        () {
          final entrypoint = 'replace';
          final params = {'string': 'value'};

          test('it throws a simulationFailed error', () async {
            // TODO(hawkbee): throw custom error instead ?
            expect(
              () => subject(params, entrypoint),
